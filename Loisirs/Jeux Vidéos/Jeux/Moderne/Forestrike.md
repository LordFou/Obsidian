---
type: game
---

# Forestrike

## Informations Générales

- **Année de sortie** : 2025 (?)
- **Développeur** : 
	- Studio : [[Skeleton Crew]]
	- Personnalités : 
		- [[Victor Moison]] (Ecriture)
- **Éditeur** : 
- **Plateformes** : 
- **Franchise** : 
- **Genre** : [[Combat]], [[Roguelike]], [[Puzzle]]
- **Résumé** : 

## Détails Techniques
- **Moteur de jeu** : 
- **Graphismes** : 2D, [[Pixel Art]], jolie DA dans l'introduction
- **Audio** : (Type de musique, Bande-son notable)
- **Réception critique** : (Notes, Awards, Réactions de la presse)
- **Gameplay** :
	- 4 zones à parcourir avec un boss de fin
	- Chaque zone augmentent le pool des techniques de combat
	- Chaque type d'ennemi a toujours les mêmes techniques à apprendre (esquive, garde, maniement d'armes,...), il faut donc apprendre les types d'ennemis et appliquer les technique des différentes écoles du héros.
	- On choisit une école en début de run mais au fur et à mesure du périple, on peut accéder à des compétences des autres écoles.
	- Chaque combat = 1 écran = 1 arène fermée
	- Début de jeu avec 3 PV => très dur !
	- En réalité on peut préméditer les combats et les actions pour trouver la solution de l'étape. Ensuite on passe en temps réel et il faut refaire ce que l'on a fait en préméditation sauf que là, on perd réellement les PV
- **Durée estimée** : 

## Histoire et Univers
- **Lieu(x)** : 
	- Contrée fictive asiatique
- **Epoque** : 
- **Personnages principaux** : 
	- Yu
- **Ennemis** :
- **Thèmes abordés** : 
	- [[Arts Martiaux]]
- **Influencé par** :
	- Les films d'arts martiaux
- **A inspiré** : 
- **Comparable à** :
	- [[Sifu]] (pour les combats)
## Anecdotes et Curiosités
- 
## Liens et Ressources

| Description | URL |
| ----------- | --- |
|             |     |
